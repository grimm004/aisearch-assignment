...
def read_file_into_string(input_file, from_ord, to_ord):
    ...
    output_string = ""
    while current_char != "":
        if from_ord <= ord(current_char) <= to_ord:
...
added_note = "The A* heuristic here uses a greedy completion to find a tighter over-estimation of the path cost to a " \
             "goal state. "
...
def h1(child_state):  # Greedy completion
    ...
    while len(child_state) < len(distance_matrix):
        ...
        path_cost += cost
    path_cost += distance_matrix[child_state[0]][child_state[-1]]
    return path_cost
        ...
        if len(state) == len(distance_matrix):
            ...
            for i in range(len(distance_matrix)):
                if i not in state:
                    ...
                    h = h1(child_state.copy())
                    new_node = (new_id, child_state, parent_id, g, depth + 1)
                    nodes.append(new_node)
                    heapq.heappush(fringe, (g + h, new_id, new_node))
